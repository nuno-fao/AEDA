...
FuelCar::FuelCar(std::string brand, std::string model, float price, float tank, float l_100km): Car(brand,model), tank(tank), l_100km(l_100km){
    setPrice(price);
};
...
ElectricCar::ElectricCar(std::string brand, std::string model, float price, float battery, float kWh_100km): Car(brand,model), battery(battery), kWh_100km(kWh_100km){
    setPrice(price);
}
